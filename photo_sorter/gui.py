...
class SorterApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.verify_registry_var = tk.BooleanVar(value=True)
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Checkbutton(
            mode_frame,
            text="Быстрый режим (QR -> полный кадр -> повороты)",
            variable=self.fast_mode_var,
        ).pack(side="left", padx=10)
        ttk.Checkbutton(
            mode_frame,
            text="Проверять по реестру при выборе номера",
            variable=self.verify_registry_var,
        ).pack(side="left", padx=10)
    ...
    def _config_from_ui(self) -> SortConfig:
        return SortConfig(
            ...
            verify_by_registry=self.verify_registry_var.get(),
        )
